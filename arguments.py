...
def add_general_args(parser):
    ...
    group.add_argument('--constant_decay', type=int, default=None,
    ...
    group.add_argument('--load_optim', action='store_true',
                        help='load most recent optimizer to resume training')
    group.add_argument('--save_iters', type=int, default=10000, metavar='N',
                        help='save current model progress interval')
    group.add_argument('--save_optim', action='store_true',
                        help='save most recent optimizer')
    group.add_argument('--fp16', action='store_true',
                        help='Run model in pseudo-fp16 mode (fp16 storage fp32 math).')
    group.add_argument('--dynamic_loss_scale', action='store_true',
                        help='Dynamically look for loss scalar for fp16 convergance help.')
    group.add_argument('--no_weight_norm', action='store_true',
                        help='Add weight normalization to model.')
    group.add_argument('--loss_scale', type=float, default=1,
    ...
    group.add_argument('--trust_coeff', type=float, default=.02,
                        help='trust coefficient value for LARC optimizer')
    group.add_argument('--chkpt_grad', action='store_true',
...
def add_unsupervised_data_args(parser):
    ...
    data_group = parser.add_argument_group('language modeling data options')
    data_group.add_argument('--seq_length', type=int, default=256,
                             help="Maximum sequence length to process (for unsupervised rec)")
    data_group.add_argument('--eval_seq_length', type=int, default=256,
    ...
    data_group.add_argument('--persist_state', type=int, default=1,
                             help='0=reset state after every sample in a shard, 1=reset state after every shard, -1=never reset state')
    data_group.add_argument('--train-iters', type=int, default=1000,
                             help="""number of iterations per epoch to run training for""")
    data_group.add_argument('--eval-iters', type=int, default=100,
                             help="""number of iterations per epoch to run validation/test for""")
    data_group.add_argument('--stlr-cut-frac', type=float, default=None,
...
def add_classifier_data_args(parser):
    data_config, data_group = configure_data(parser)
    data_group.add_argument('--seq_length', type=int, default=256,
                             help="Maximum sequence length to process (for unsupervised rec)")
    data_group.add_argument('--lazy', action='store_true',
